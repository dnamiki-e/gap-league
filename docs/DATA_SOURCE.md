...
> **重要: 試合は打ち切らずに全件返すこと。**
> 締切の解決（「ラウンド16の初戦キックオフ1時間前」など）と `Match` テーブルの
> 保存に、シーズンの全日程が要る。上位N件だけ返す実装だと、
> シーズン後半の試合が保存されず締切が永久に解決できない。
>
> 実測（2026-09-04）: PL 2026 シーズンで football-data.org 直叩きは **380件**
> （2027-05-30 まで）を返したが、中継サーバ経由では **100件**（2026-11-07 まで）
> しか返らなかった。中継サーバを使う場合は、この点を必ず確認すること。
